...
默认使用的是 `ChainCache([MemoryLRU(), DiskJoblib()])`，一次 `get(fn_name, hash_value)` 的路径大致是：
...
- **② 链式查找**：先 MemoryLRU，未命中再 DiskJoblib。  
...
### 2.3 DiskJoblib.get（cache.py 126-148）——主要瓶颈

```python
p = self._path(fn_name, hash_value)   # Path 拼接，便宜
try:
    if p.stat().st_size <= self.small_file:   # ④ stat() 系统调用
        with p.open("rb") as fh:              # ⑤ 打开文件
            return True, pickle.load(fh)      # ⑥ 整文件读 + 反序列化
    return True, joblib.load(p)               # 大文件同理
except FileNotFoundError:
    return False, None
```

- **④ stat()**：每个 key 一次 `stat()`。百万节点、且内存未命中时，就是百万次 metadata 查询，在机械盘或网络盘上会非常慢。  
- **⑤ 打开文件**：存在则 open，又是一次 syscall + 可能的内核/文件系统开销。  
- **⑥ 反序列化**：`pickle.load(fh)` / `joblib.load(p)` 会读完整文件并反序列化整个对象。  
  - 即使返回值在 **build_graph 里被丢弃**（只用 `[0]`），当前实现也 **一定会做「读文件 + 反序列化」**，这是设计上的浪费。

因此：  
- **内存命中**：单次 get 很快。  
- **内存未命中、磁盘命中**：单次 get = stat + open + 读文件 + unpickle，可能几 ms～几十 ms，且与节点数线性叠加。  
- **磁盘也未命中**：至少 stat（+ 可能 open 失败），仍然有 syscall 成本。
...
- 所以 DiskJoblib 每次命中都会：stat → open → **整文件读 + 完整 unpickle**，再把 value 返回；调用方只用 `[0]`，**value 被立刻丢弃**。  
...
- 大量 `stat()` + `open` + 读文件，在机械盘或网络盘上会变成大量随机/小文件 IO，延迟高。  
...
- 主要原因可以概括为：  
  ...
  3. **单次成本** 在磁盘命中时 = stat + open + read + unpickle，在慢盘上尤其高。  
  4. **锁与回填** 等固定开销被调用次数放大。

可行的优化方向包括：

- **为 Cache 增加「仅判存在」的接口**（例如 `contains(fn_name, hash_value) -> bool`），在 `build_graph` 里用该接口代替 `get(..., node._hash)[0]`，避免在构图阶段做任何反序列化。  
- **DiskJoblib 实现 contains**：用 `Path.exists()` 或一次轻量 stat 判断文件是否存在，不 open、不 load。  
- **ChainCache.contains**：沿链依次查各层 contains，命中则返回 True，不加载 value，不回填（或仅在需要时对上层做轻量标记）。  
- 若仍保留「构图时用 get」的兼容路径，可考虑在 DiskJoblib 中为「只关心 hit」的调用提供可选轻量路径（例如内部 `get(exists_only=True)`），避免无条件 pickle.load。
