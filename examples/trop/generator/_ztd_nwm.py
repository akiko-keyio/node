...
from trop.generator.ztd_met import zhd_saastamoinen, zwd_saastamoinen
from trop.coordinate.geoid import GeoidHeight
from trop.coordinate.height_convert import geopotential_to_geometric
from trop.utils.Interprepter import (
    LinearInterpolator,
...
class ZTDNWMGenerator:
    ...
    # ---------------- Check Function ---------------- #
    def check_site_vertical_status(self) -> pd.DataFrame:
        """
        诊断函数：判断每个站点（在每个时刻）位于模型最低层之上还是之下。

        该函数会执行 run() 流程的前 6 步（读取、格式化、水平插值、单位换算、
        位势高转正高、正高转椭球高），从而获取精确的 NWM 模型椭球高。

        Returns
        -------
        pd.DataFrame
            包含以下列的 DataFrame:
            - time, number, site, site_index: 索引信息
            - station_alt: 站点椭球高 (米)
            - model_min_h: 该位置 NWM 模型的最低层高程 (米)
            - height_diff: station_alt - model_min_h (负值表示站点在模型下方)
            - status: 字符串标识 ("BELOW_MODEL" 或 "INSIDE_MODEL")
        """
        logger.info("Starting vertical status check (running steps 1-6)...")

        # 1. 确保数据已加载并执行前处理 (Steps 1-6)
        # 注意：为了保证完全一致，我们按顺序调用相同的方法
        if self.ds is None:
            self.read_met_file()

        self.format_dataset()
        self.horizontal_interpolate()  # 将网格插值到站点位置
        self.quantify_met_parameters()  # 单位标准化
        self.geopotential_to_orthometric()  # 转正高
        self.orthometric_to_ellipsoidal()  # 转椭球高 (关键步骤)

        logger.info("Geometry calculation finished. Comparing altitudes...")

        # 2. 提取计算所需的维度名称
        # 通常是 'pressure_level'，但也可能是用户指定的其他维度
        v_dim = self.vertical_dimension
        if v_dim not in self.ds.dims:
            # 如果尚未重命名，尝试找常见的垂直维
            for d in ["pressure_level", "isobaricInhPa", "level"]:
                if d in self.ds.dims:
                    v_dim = d
                    break

        if v_dim not in self.ds.dims:
            raise ValueError(f"Could not find vertical dimension '{self.vertical_dimension}' in dataset.")

        # 3. 计算模型最低高程
        # 在垂直维度上取最小值 (h).
        # 注意：h 随 number, time, site_index 变化
        model_min_h = self.ds["h"].min(dim=v_dim)

        # 4. 获取站点高程
        # 此时 self.ds['alt'] 应该已经通过 horizontal_interpolate 赋值了
        # 它的维度通常是 (site_index,)，xarray 会自动处理广播
        station_alt = self.ds["alt"]

        # 5. 计算差异 (站点高 - 模型底)
        # diff < 0 表示站点在模型下方 (需外推)
        # diff >= 0 表示站点在模型内部 (内插)
        diff = station_alt - model_min_h

        # 6. 转换为 DataFrame 以便阅读
        # 我们保留 diff 和 min_h，利用 xarray 的广播机制展开所有维度
        check_ds = xr.Dataset({
            "station_alt": station_alt,
            "model_min_h": model_min_h,
            "height_diff": diff
        })

        # 添加 site 名称以便人类阅读 (如果存在)
        if "site" in self.ds:
            check_ds["site"] = self.ds["site"]

        df = check_ds.to_dataframe().reset_index()

        # 7. 增加状态标识列
        # 为了处理浮点数精度，我们认为差异小于 -0.001 米即为下方
        df["status"] = np.where(
            df["height_diff"] < -0.001,
            "BELOW_MODEL (Extrapolation)",
            "INSIDE_MODEL (Interpolation)"
        )

        # 简单的统计报告
        n_below = (df["height_diff"] < -0.001).sum()
        n_total = len(df)
        max_depth = df["height_diff"].min()

        logger.info(f"Check Report: {n_below}/{n_total} cases are below model bottom.")
        if n_below > 0:
            logger.warning(f"Deepest station is {abs(max_depth):.2f}m below model terrain.")
            logger.warning("For these cases, strictly verify if Log-Cubic extrapolation is appropriate.")

        # 清理不需要的列 (根据你的 df 结构可能需要调整)
        cols = ["time", "site", "number", "station_alt", "model_min_h", "height_diff", "status"]
        # 仅返回存在的列
